...
TGroup.can.draw = function(status) {
	delete this.caret
//	this.clear(this.color.fore, this.color.back)
	for (var i = 0; i < this.items.length; i++)
		if (this.items[i].visible())
			this.items[i].draw({ active: this.checkActive(this.items[i]), focused: this.checkFocus(this.items[i]), disabled: (status.disabled == true) || (this.disabled == true) })
	for (var i = 0; i < this.items.length; i++) {
...
TGroup.can.onKey = function(key, down, physical) {
	if (this.actor != undefined && this.actor.onKey != undefined) return this.actor.onKey(key, down, physical)
}
TGroup.can.onChar = function(ch) {
	if (this.actor != undefined && this.actor.onChar != undefined) return this.actor.onChar(ch)
}
